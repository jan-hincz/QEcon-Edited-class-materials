...
my_ngm = NGMProblem2(n=200) #defined at ca. line 4
...
my_ngm_low_γ = NGMProblem2(γ=0.5,n=300) #defined at line ca. 7
v_low_γ, σ_low_γ, iter_low_γ, err_low_γ, v_history_low_γ = vfi(my_ngm_low_γ) #defined at line ca. 60

my_ngm_high_γ = NGMProblem2(γ=2.0,n=300)
...
a = findfirst(x->k_path_low_γ[end] -x<0.5*(k_path_low_γ[end]-k_path_low_γ[1]),my_ngm_low_γ.k_grid)
b = findfirst(x->k_path_high_γ[end] -x<0.5*(k_path_high_γ[end]-k_path_high_γ[1]),my_ngm_high_γ.k_grid)
periods = [a,b]
γ_vec = [0.5,2.0]
data = hcat(periods, γ_vec)

header = (["time of half-convergence","γ"],["periods",""]) #[columns], [subtitles]
